...
public class ReferView extends View implements DatabaseListener {
	...
	private void init() {
		...
		iqIndicator = (BitmapDrawable) getResources().getDrawable(
				R.drawable.tianchong);
		bmp = iqIndicator.getBitmap();

		iqCover = (BitmapDrawable) getResources()
				.getDrawable(R.drawable.jieguo);
		leftX = -bmp.getWidth();
	}
	...
	public void onDraw(Canvas c) {
		...
		if (resizedBmp == null && bmp != null) {
			...
			resizedBmp = Bitmap.createBitmap(bmp, 0, 0, bmpWidth, bmpHeight,
					matrix, true);
			...
			coverBmp = Bitmap.createBitmap(iqCover.getBitmap(), 0, 0,
					coverBmpWidth, coverBmpHeight, coverMatrix, true);
		}
